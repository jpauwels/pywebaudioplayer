...
def wavesurfer(audio_path=None, controls={}, display={}, behaviour={}, samples=None):
    ...
    wavesurfer{id}.load("{path}");
        ...
        id=unique_id, path=audio_path, cursor=display['cursor_colour'], progress=display['played_wave_colour'], wave=display['unplayed_wave_colour'], split=js(not behaviour['mono']), height=display['height'], norm=js(behaviour['normalize']),
    ...
    for track in tracks:
        ...
        {{
            ...
            "soloed": {}
                      ...
                      js(track['mute']) if 'mute' in track else 'false',
                      js(track['solo']) if 'solo' in track else 'false',)
    html_code += '''
    ]).then(function() {
    });
    </script>
...
def trackswitch(tracks, text='', seekable_image=None, seek_margin=None, mute=True, solo=True, globalsolo=True, repeat=False, radiosolo=False, onlyradiosolo=False, spacebar=False, tabview=False):
    ...
    '''.format(unique_id, js(mute), js(solo), js(globalsolo), js(repeat), js(radiosolo), js(onlyradiosolo), js(spacebar), js(tabview))
